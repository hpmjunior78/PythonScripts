...
for speed in List_of_speeds:
	...
	for lr in List_of_LossRates:
		...
		file_mask = '*' + reference_node + '*.Periodic.csv.done'
		## Reading the periodic saved trace
		file = glob.glob(final_path + file_mask)
		# Periodic_saved data frames are already normalized on TimeStamp value
		Sol01_Prdsvg_df_tmp = pd.read_csv(file[0])
				
		## Cropping out unnecessary columns
